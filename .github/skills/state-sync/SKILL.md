...
printf '%s' '{"phase":"planning","requirements":{"status":"approved"}}' | \
  node .github/hooks/scripts/workflow_hook.cjs update-state
```
...
If an agent or prompt does not expose direct file-edit tools, use the host session's available terminal or command tool to call this API instead of trying to edit the state file indirectly.
