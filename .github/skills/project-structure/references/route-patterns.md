...
The purpose of `src/shared/routes` is to be the single source of truth for
**user-facing path builders**. The `src/app` tree is still the source of
rendering structure and Next.js file conventions, but application code should
consume route helpers instead of duplicating raw path strings.
...
- paths consumed by links, redirects, buttons, navigation, and route-aware
  logic
...
- if it only exists to organize `src/app`, keep it in the `app/` tree, not in
  the route helper API
...
These App Router features affect rendering structure, not necessarily the
public URL API.

## 3. Route Groups Should Reflect URL Meaning, Not Filesystem Syntax

`src/shared/routes` may use `root`, `public`, and `private` as organizational
groups, but the generated path must reflect the real URL, not the route-group
folder name.
...
Route-group folders help organize `src/app`, but they do not appear in the URL
and should not leak into route helpers.

## 4. Keep Locale Handling Out of Route Paths

This project uses locale-aware navigation wrappers from
`@/shared/lib/navigation`.

Because locale handling is already applied there:

- route helpers should return locale-neutral paths such as `"/"` or
  `"/sign-in"`
- do not add `"/th"` or `"/en"` manually in route helpers
- do not add locale parameters to normal route builder APIs unless a route
  genuinely needs them for a different reason
...
- use `root`, `public`, and `private` only as organizational route-helper
  groups
- keep one route helper object per leaf folder
- use a leaf `index.ts` inside each route folder
- keep the top-level `src/shared/routes/index.ts` as the public aggregator
  for the route API
...
When an App Router segment is dynamic, the route helper should accept explicit
parameters that match the user-facing URL contract.
...
Use catch-all helpers only when the URL contract truly needs arbitrary nested
segments.
...
If the route can be modeled with a clearer fixed helper, prefer the clearer
API.

Do not introduce catch-all route helpers just because the App Router
filesystem supports them.

## 9. Parallel and Intercepting Routes

Most parallel-route and intercepting-route details should stay out of
`shared/routes`.
...
If a modal or intercepted route corresponds to a real navigable page such as
`/photos/123`, model only the real URL:
...
- `pages`
- `paths`
- names copied from filesystem-only conventions such as `(private)` or
  `@modal`
...
- framework configuration where the shared route helper layer is not
  appropriate
...
- modeling filesystem structure more literally than the user-facing URL
  requires
...
- Should this be modeled in `shared/routes`, or is it only an `app/`
  filesystem concern?
