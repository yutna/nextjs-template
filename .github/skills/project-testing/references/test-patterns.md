...
This guide defines how to write and organize tests in this repository,
including:

- shared test support in `src/test`
- colocated `*.test.ts` and `*.test.tsx` files across `src/shared` and
  `src/modules`

Use this guide to keep tests consistent with the repository's Vitest, Testing
Library, and server-first Next.js architecture.

## Decide Whether Code Belongs in `src/test` or Beside the Unit
...
## Testing Stack and Baseline Assumptions
...
## Responsibility of `src/test`

`src/test` is for reusable test support only.

### `setup.ts`

Use `src/test/setup.ts` for global environment configuration that should apply
...
or update a shared helper in `src/test` instead of duplicating wrappers in
many files.
...
This repository uses server components by default. Test async server components
by calling them directly and awaiting the result.
...
Use this when needed so test environments can import server-only guarded
modules safely.

## Action and Lib Tests

Non-React units such as actions and library helpers should usually use
`*.test.ts`.
...
Do this when multiple tests share the same mocked module and call history
matters.
