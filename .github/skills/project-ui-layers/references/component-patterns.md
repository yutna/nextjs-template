...
This reference defines how to write and organize UI components inside:

- `src/shared/components`
- `src/modules/<module-name>/components`

Use `components/` for stateless or presenter-oriented UI pieces. These folders
are for components whose main job is rendering, visual composition, and exposing
a clear props API, not owning feature orchestration or heavy stateful behavior.
...
- if the file is mostly about how something looks and renders, `components/` is
  usually
  correct
- if the file is mostly about how a feature works, it likely belongs outside
  `components/`
...
This matches the repository direction that view and logic should be separated
even when the extracted logic is only locally reused.

## 3. Boundaries with nearby UI layers

This guide stays focused on `components`, so nearby layers are mentioned here
...
- `layout` then `screen` then `container` then `component`
- `component` then private child presenter components
...
Use: `src/modules/<module-name>/components`
...
- colocate Storybook files such as `component-name.stories.tsx` in the same
  component folder
- keep tests adjacent to the component they cover
- allow private child components inside the same folder when they only support
  that component
- allow colocated `constants.ts` when the values belong only to that component
  folder
- do not create parent barrel files for `src/shared/components` or
  `src/modules/<module-name>/components`

If a helper file becomes broadly reusable or logic-heavy, move it to a more
appropriate folder instead of letting the component folder become a grab bag.
...
- name the public component after what it presents, not where it happens to be
  used
...
- making callers understand internal state machinery just to render the
  component
...
Components can be server or client components, but they should stay presenter-
oriented in either case.

- use server components by default when the component only needs server-safe
  rendering
- add `"use client"` only when the component needs hooks, browser APIs, or
  client-only
  interactivity
...
- do not keep heavy client state in a presenter component when a hook or
  container would make
...
- `not-found.tsx` stays server-oriented and delegates one client interaction to
  a child
  presenter
...
- styles are global cross-cutting concerns — move to `shared/styles`
- values become broader than one component folder — move to `constants/`
- logic becomes reusable or test-heavy — move to `hooks/` or `utils/`
...
These concerns usually belong in a hook or container, with the component
receiving the prepared values and callbacks:
...
Before adding a file to `shared/components` or `modules/<module-
name>/components`, check:

- is this primarily a presenter or stateless UI concern?
- is `components/` a clearer home than `containers/`, `screens/`, `layouts/`, or
  `hooks/`?
