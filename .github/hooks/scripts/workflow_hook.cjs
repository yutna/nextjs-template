...
function updateStateUsageText() {
  return [
    'Usage:',
    "  printf '%s' '{\"phase\":\"planning\",\"requirements\":{\"status\":\"approved\"}}' | node .github/hooks/scripts/workflow_hook.cjs update-state",
    '  node .github/hooks/scripts/workflow_hook.cjs update-state phase=implementation implementation.status=in-progress',
    '',
    'Notes:',
...
function workflowStatePath(cwd) {
  return path.join(cwd, '.github', 'workflow-state.json');
}

function runtimeDir(cwd) {
  const workspaceHash = crypto.createHash('sha1').update(path.resolve(cwd)).digest('hex').slice(0, 12);
  const dirPath = path.join(os.tmpdir(), 'copilot-workflow-hooks', workspaceHash);
...
function logPath(cwd) {
  return path.join(cwd, '.github', 'hooks', 'workflow-hook.log');
}
...
function commandReferencesStateFile(commandText) {
  ...
  return /(^|[\s"'`])(?:\.\/)?\.github\/workflow-state\.json\b/.test(commandText);
}
...
function collectStrings(value) {
  ...
  if (isPlainObject(value)) {
    const items = [];
    for (const key of ['path', 'file', 'filePath', 'source', 'destination', 'target', 'files', 'paths', 'command', 'commands']) {
      if (key in value) {
...
function collectPathStrings(value) {
  ...
  if (isPlainObject(value)) {
    const items = [];
    for (const key of ['path', 'file', 'filePath', 'source', 'destination', 'target', 'files', 'paths']) {
      if (key in value) {
...
function isGovernancePath(filePath) {
  return filePath === 'AGENTS.md' || filePath.startsWith('.github/');
}
...
function sessionContext(event) {
  ...
  if (!stateFileExists || (phase === 'discovery' && !activeState.taskId && !activeState.taskSummary)) {
    message +=
      ' Fresh bootstrap state detected. Start with the discover-requirements prompt, then persist task details through the workflow state API.';
  }
...
function workflowGuard(event) {
  ...
  if (stateErrors.length > 0 && !stateTargeted && !readOnlyCommand && !stateApiCommand) {
    ...
    return emitPreToolDecision(
      'ask',
      'Workflow state is invalid. Repair .github/workflow-state.json before risky actions.',
      stateErrors.join('; '),
    );
  }
  ...
  if (stateTargeted && isEditTool(toolName)) {
    if (phaseIndex(currentPhase) >= phaseIndex('implementation')) {
      ...
      return emitPreToolDecision(
        ...
        "Use `printf '%s' '{...}' | node .github/hooks/scripts/workflow_hook.cjs update-state` or `node .github/hooks/scripts/workflow_hook.cjs update-state phase=implementation implementation.status=in-progress` so validation and transition rules stay consistent.",
      );
    }
  ...
  if (shellStateWriteAttempt) {
    ...
    return emitPreToolDecision(
      'deny',
      'Direct shell writes to .github/workflow-state.json are blocked. Use the workflow state API instead.',
      "Use `printf '%s' '{...}' | node .github/hooks/scripts/workflow_hook.cjs update-state` or `node .github/hooks/scripts/workflow_hook.cjs update-state phase=implementation implementation.status=in-progress`.",
    );
  }
  ...
  if (!governanceOnly && !['clarified', 'approved'].includes(requirementsStatus) && (isEditTool(toolName) || riskyCommand)) {
    ...
    return emitPreToolDecision(
      ...
      'Use the discovery workflow and update .github/workflow-state.json before implementation work.',
    );
  }
...
function postEditChecks(event) {
  ...
  const message = [
    ...
    'Files changed. Update .github/workflow-state.json with any remaining phase or blocker changes.',
...
function stopGate(event) {
  ...
  if (retryCount >= MAX_RETRY_COUNT && blockedItems.length === 0) {
    const reason = 'Retry budget is exhausted, but no blocked item was recorded in workflow state.';
    logEvent(cwd, 'warning', 'Stop blocked because retry exhaustion was not recorded', { retryCount });
    return emitStopDecision('block', reason);
  }

  if (implementationStopSatisfied(state)) {
    logEvent(cwd, 'info', 'Stop allowed after implementation completion', {
      phase,
      implementationStatus: lower(implementation.status || ''),
    });
    return emitStopDecision('allow');
  }
  ...
  if (['implementation', 'quality-gates', 'verification'].includes(phase) || touched.length > 0) {
    if (!allGatesGreen(state)) {
      ...
      logEvent(cwd, 'warning', 'Stop blocked because gates are incomplete', {
