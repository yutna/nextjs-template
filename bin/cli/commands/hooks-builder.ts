  ...
  for (const hook of HOOKS_DEFINITIONS) {
    for (const copilotHook of hook.copilotHooks) {
      ...
      configs[configKey] = {
        ...
        notes: `Generated by hooks-builder.ts | ${new Date().toISOString()}`,
        version: 1,
      };
    }
  }
...
// CLI interface
export const command = "hooks:build";
export const help = `
Builder for hook configurations.

DESCRIPTION
  Generates both Claude Code (.claude/settings.json) and GitHub Copilot (.github/hooks/*.json)
  hook configurations from a single TypeScript source of truth.

  This ensures consistency between Claude and Copilot hook definitions and eliminates
  maintenance burden from keeping separate JSON files in sync.

USAGE
  npm run hooks:build

HOOKS DEFINED
${HOOKS_DEFINITIONS.map((h) => `  • ${h.id}: ${h.description}`).join("\n")}
